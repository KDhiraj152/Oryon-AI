...
class WriteBehindQueue:
    ...
    def put(self, key: str, value: Any) -> None:
        ...
        O(1) write coalescing via key index map.
        """
        with self._lock:
            # O(1) write coalescing via index lookup
            if key in self._key_index:
                idx = self._key_index[key]
                if idx < len(self._queue) and self._queue[idx][0] == key:
                    self._queue[idx] = (key, value)
                    return  # Coalesced
                else:
                    # Index stale after flush — remove and re-add
                    del self._key_index[key]

            self._key_index[key] = len(self._queue)
    ...
    def _flush(self) -> None:
        ...
        with self._lock:
            ...
            # Rebuild key index for remaining items
            self._key_index = {k: i for i, (k, _) in enumerate(self._queue)}
        ...
        if self._sync_flush_callback:
            try:
                ...
                logger.warning("Sync write-behind flush failed: %s", e)
                return
        ...
        if self._async_flush_callback:
            try:
                ...
                logger.warning("Async write-behind flush failed: %s", e)
...
class L1Cache:
    ...
    def _rebuild_bloom_filter(self) -> None:
        ...
        logger.debug("[L1Cache] Rebuilt bloom filter with %s keys", len(current_keys))
...
class L2Cache:
    ...
    def _get_sync_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.warning("Redis sync client error: %s", e)
                self._errors += 1
                return None
        return self._client
    ...
    async def _get_async_client(self):
        ...
        if self._async_client is None:
            try:
                ...
                logger.warning("Redis async client error: %s", e)
                self._errors += 1
                return None
    ...
    async def get(self, key: str) -> Any | None:
        ...
        try:
            ...
            logger.warning("Redis async get error: %s", e)
            self._errors += 1
            return None
    ...
    async def set(self, key: str, value: Any, ttl: int | None = None) -> bool:
        ...
        try:
            ...
            logger.warning("Redis async set error: %s", e)
            self._errors += 1
            return False
    ...
    def get_sync(self, key: str) -> Any | None:
        ...
        try:
            ...
            logger.warning("Redis sync get error: %s", e)
            self._errors += 1
            return None
    ...
    def set_sync(self, key: str, value: Any, ttl: int | None = None) -> bool:
        ...
        try:
            ...
            logger.warning("Redis sync set error: %s", e)
            self._errors += 1
            return False
    ...
    def _init_db(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug("SQLite migration check: %s", e)
    ...
    def get_sync(self, key: str) -> Any | None:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite get error: %s", e)
                return None
    ...
    def set_sync(self, key: str, value: Any) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite set error: %s", e)
                return False
    ...
    def _mset_sync(self, items: dict[str, Any]) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite mset error: %s", e)
                return False
    ...
    def delete_sync(self, key: str) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite delete error: %s", e)
                return False
    ...
    def mget_sync(self, keys: list[str]) -> dict[str, Any]:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite mget error: %s", e)
    ...
    def cleanup_expired(self) -> int:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite cleanup error: %s", e)
                return 0
    ...
    def vacuum(self) -> None:
        ...
        with self._lock:
            try:
                ...
                logger.warning("SQLite vacuum error: %s", e)
...
class UnifiedCache:
    ...
    async def _flush_writes(self, items: list[tuple[str, Any]]) -> None:
        ...
        try:
            ...
            logger.warning("[UnifiedCache] L2 batch write failed: %s", e)
        ...
        try:
            ...
            logger.warning("[UnifiedCache] L3 batch write failed: %s", e)
