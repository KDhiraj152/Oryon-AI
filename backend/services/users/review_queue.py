...
class ResponseReviewQueue:
    ...
    def _store_flagged(self, flagged: FlaggedResponse):
        ...
        if self._redis:
            try:
                ...
                logger.warning("Redis store failed, using fallback: %s", e)
    ...
    def review(
        ...
        if not flagged:
            logger.warning("Response %s not found for review", response_id)
            return None
        ...
        logger.info("Response %s reviewed: status=%s", response_id, status.value)
        return flagged
    ...
    def get_pending_count(self) -> int:
        ...
        if self._redis:
            try:
                return cast(int, self._redis.llen(self.QUEUE_KEY))
...
def get_review_queue(redis_client=None) -> ResponseReviewQueue:
    """Get or create the review queue singleton."""
