...
def _count_tokens(text: str) -> int:
    ...
    if _tiktoken_enc is None:
        try:
            ...
            _tiktoken_enc = None  # sentinel: don't retry (no tiktoken available)
...
try:
    from ...core.policy import PolicyMode, get_policy_engine
...
class AIEngine:
        ...
        with self._lock:
            ...
            if _POLICY_AVAILABLE:
                try:
                    ...
                    logger.warning("PolicyEngine initialization failed: %s", e)
            ...
            try:
                ...
                logger.warning("SafetyGuard initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("SelfOptimizer initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("SafetyPipeline initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("RetrievalLoop initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("ResourceScheduler initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("SpeculativeDecoder initialization skipped: %s", e)
            ...
            try:
                ...
                logger.warning("ContextAllocator initialization skipped: %s", e)
    ...
    def _get_collaborator(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not load Model Collaborator: %s", e)
    ...
    def _get_sandbox(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not load sandbox: %s", e)
    ...
    def _get_self_optimizer(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get self-optimizer: %s", e)
    ...
    def _get_safety_pipeline(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get safety pipeline: %s", e)
    ...
    def _get_retrieval_loop(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get retrieval loop: %s", e)
    ...
    def _get_feedback_learner(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get feedback learner: %s", e)
    ...
    def _get_resource_scheduler(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get resource scheduler: %s", e)
    ...
    def _get_speculative_decoder(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not get speculative decoder: %s", e)
    ...
    def _get_context_allocator(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not create context allocator: %s", e)
    ...
    async def _verify_response_safety(
        ...
        try:
            ...
            logger.warning("Safety pipeline check failed: %s", e)
            return response, True, None
    ...
    def _record_optimization_metrics(
        ...
        try:
            ...
            logger.debug("Failed to record optimization metrics: %s", e)
    ...
    def _get_evaluator(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Could not load Semantic Evaluator: %s", e)
    ...
    async def chat(
        ...
        if fast_result:
            ...
            return self.formatter.format_response(
                ...
                sources=[],
                tokens_prompt=0,
                tokens_completion=_count_tokens(fast_result),
        ...
        if conversation_id:
            ...
            context = self.context_manager.get_or_create(str(uuid_module.uuid4()), user_id)
    ...
    def _allocate_compute_resources(
        ...
        if context_allocator:
            try:
                ...
                logger.debug("Context allocation failed: %s", e)
        ...
        if resource_scheduler:
            try:
                ...
                logger.debug("Resource scheduling failed: %s", e)
    ...
    async def _perform_standard_rag_search(
        ...
        if not rag_result:
            logger.info("RAG returned no results for: %s...", message[:50])
            return [], "", True
    ...
    async def _generate_response(
        ...
        try:
            ...
            result = GenerationResult(
                ...
                tokens_completion=_count_tokens(response_text),
                latency_ms=latency_ms,
    ...
    def _fetch_stream_rag_context(self, message: str, use_rag: bool) -> tuple[str, bool]:
        ...
        try:
            ...
            logger.warning("RAG search failed in streaming: %s", e)
            return "", True
    ...
    async def _stream_with_timeout(
        ...
        from backend.ml.inference.unified_engine import GenerationConfig as InferenceGenConfig
        ...
        async for token in llm.generate_stream(
            ...
            if now - last_token_time > timeout:
                logger.warning("Stream generation timed out after %ss between tokens", timeout)
                break
            last_token_time = now
            yield token
    ...
    async def _run_generation(
        ...
        if speculative_decoder and speculative_decoder.is_available():
            try:
                ...
                logger.debug("Speculative decoding failed, falling back: %s", e)
        ...
        try:
            ...
            if hasattr(llm, "generate_async") and asyncio.iscoroutinefunction(
                ...
                from backend.ml.inference.unified_engine import (
                    GenerationConfig as InferenceGenConfig,
                )
            ...
            logger.warning("Generation timed out after %ss", config.timeout_seconds)
    ...
    def _build_system_prompt(
        ...
        if is_unrestricted:
            ...
            base_prompt = (
                "You are Oryon, an intelligent AI assistant. "
    ...
    def _build_prompt(
        ...
        # System prompt with RAG context embedded
    ...
    def _should_use_rag(self, message: str, intent: Optional["Intent"] = None) -> bool:
    ...
    def _safe_math_eval(expr: str):
        ...
        Only allows: numbers, +, -, *, /, **, unary -, parentheses.
    ...
    def _format_sse_complete(self, text: str, latency_ms: float) -> str:
        ...
        return f"data: {json.dumps({'type': 'complete', 'data': {'message_id': str(uuid_module.uuid4()), 'text': text, 'latency_ms': latency_ms}})}\n\n"
    ...
    def record_feedback(
        ...
        try:
            import hashlib as _fb_hashlib

            from ...core.optimized.self_optimizer import UserFeedback

            feedback = UserFeedback(
                query_hash=_fb_hashlib.md5(
                    (query or "").encode(), usedforsecurity=False
                ).hexdigest()[:12],
                feedback_type=feedback_type or "helpful",
            ...
            logger.debug("Recorded feedback: %s for %s", feedback_type, response_id)
            return True
        except (ImportError, ValueError, TypeError, RuntimeError) as e:
