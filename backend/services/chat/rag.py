...
try:
    from backend.core.optimized import AcceleratedSearch, get_accelerator
...
try:
    from backend.core.optimized import get_memory_pool
...
_query_cache: OrderedDict[str, tuple[list["RetrievalResult"], float]] = OrderedDict()
_query_cache_lock = create_lock()
...
def _get_cached_results(key: str) -> list["RetrievalResult"] | None:
    """Get cached results if not expired (LRU: promotes on hit)."""
    import time
...
def _set_cached_results(key: str, results: list["RetrievalResult"]) -> None:
    ...
    with _query_cache_lock:
        # Evict LRU (oldest-accessed) entries if full
...
def cleanup_rag_models():
    ...
    if _embedder_instance is not None:
        try:
            ...
            logger.error("Error unloading embedder: %s", e)
    ...
    if _reranker_instance is not None:
        try:
            ...
            logger.error("Error unloading reranker: %s", e)
...
class BGEM3Embedder:
    ...
    def __init__(self, model_id: str | None = None, device: str | None = None):
        ...
        logger.info("BGEM3Embedder initialized: %s on %s", self.model_id, self.device)
    ...
    def _load_model_impl(self):
        ...
        logger.info("Loading BGE-M3 embedding model: %s", self.model_id)
        ...
        try:
            ...
            logger.info("Loaded BGE-M3 with FlagEmbedding on %s", self.device)
    ...
    def encode(
        ...
        try:
            from backend.core.circuit_breaker import get_ml_breaker
    ...
    async def encode_with_cache(
        ...
        """
        try:
            from backend.infra.cache import get_embedding_cache
    ...
    async def encode_batch_coalesced(
        ...
        """
        try:
            from backend.core.optimized.gpu_pipeline import QueuePriority, get_gpu_scheduler
...
class BGEReranker:
    ...
    def __init__(self, model_id: str | None = None, device: str | None = None):
        ...
        self._memory_registered = False
        logger.info("BGEReranker initialized: %s", self.model_id)
    ...
    def _load_model(self):
        ...
        if MEMORY_COORDINATOR_AVAILABLE and not self._memory_registered:
            ...
            try:
                ...
                logger.error("Cannot load reranker: %s", e)
                raise
    ...
    def _load_model_impl(self):
        """Actual model loading implementation."""
        logger.info("Loading BGE Reranker: %s", self.model_id)
        ...
        try:
            ...
            self._use_cross_encoder = False
            logger.info("Loaded BGE Reranker with FlagEmbedding on %s", self.device)
...
class RAGService:
    ...
    def _verify_pgvector(self):
        ...
        try:
            ...
            logger.warning("pgvector verification skipped: %s", e)
        finally:
            session.close()
