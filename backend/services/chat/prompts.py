...
class PromptVersion:
    ...
    def render(self, **kwargs) -> str:
        ...
        if missing:
            logger.warning("Missing variables for prompt %s: %s", self.name, missing)
...
class PromptRegistry:
        ...
        "system.base": PromptVersion(
            ...
            tests=[
                PromptTest(
                    ...
                    input_text="Hello",
                    expected_contains=["Oryon", "educational"],
                ),
            ],
            changelog=[
                ChangelogEntry(
    ...
    def set_active_version(self, name: str, version: str) -> bool:
        ...
        logger.info("Set active version: %s → v%s", name, version)
        return True
    ...
    def setup_ab_test(
        ...
        logger.info("Setup A/B test for %s: %s", name, version_weights)
        return True
        ...
        for name, versions in data.get("prompts", {}).items():
            for version, prompt_data in versions.items():
                try:
                    ...
                    logger.warning("Failed to import %s v%s: %s", name, version, e)
