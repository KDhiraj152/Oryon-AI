...
class SafetyPipeline:
    ...
    TOXICITY_PATTERNS: ClassVar[list] = [
    ...
    HALLUCINATION_INDICATORS: ClassVar[list] = [
    ...
    async def verify(
        ...
        if pass_coros:
            ...
            for name, result in zip(pass_names, gathered, strict=False):
                if isinstance(result, BaseException):
                    logger.warning("Safety %s failed: %s", name, result)
                    continue
