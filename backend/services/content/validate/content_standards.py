...
class ContentStandardsLoader:
    ...
    def generate_embeddings(self) -> None:
        ...
        logger.info("Generating embeddings for %s standards...", len(self.standards))
        ...
        if texts:
            try:
                ...
                logger.info("Generated %s embeddings with BGE-M3", len(texts))
    ...
    def _get_text_embedding(self, text: str) -> np.ndarray:
        ...
        embedder = self._get_embedder()
        result: np.ndarray = embedder.encode([text])[0]
        return result
    ...
    def save_to_database(self) -> None:
        ...
        try:
            ...
            session.commit()
            logger.info("Saved %s standards to database", len(self.standards))
    ...
    def find_matching_standards(
        ...
        for standard in filtered_standards:
            if standard.embedding is None:
                text = standard.combined_text if standard.combined_text is not None else ""
                standard.embedding = self._get_text_embedding(text)
...
def initialize_content_standards(json_path: str | Path | None = None) -> ContentStandardsLoader:
    ...
    try:
        ...
        if not standards:
            ...
            logger.info("Loaded %s content standards from database", len(standards))
            # Generate embeddings for loaded standards
            loader.generate_embeddings()

    except (OSError, RuntimeError, ValueError) as e:
        logger.error("Error initializing content standards: %s", e)
