...
from backend.models import ContentStandard, ContentValidation, ProcessedContent
...
class ContentValidationService:
    ...
    async def validate_factual_accuracy(
        ...
        """
        logger.info("Validating factual accuracy for content %s", content_id)
    ...
    async def validate_language_appropriateness(
        ...
        Returns:
            ContentValidation object
        """
        logger.info("Validating language appropriateness for Grade %s", complexity_level)
    ...
    async def comprehensive_validation(
        ...
        """
        logger.info("Running comprehensive validation for content %s", content_id)
    ...
    def get_improvement_suggestions(self, content_id: str) -> list[str]:
        ...
        for validation in validations:
            ...
            elif validation.validation_type == "language" and validation.alignment_score < 0.6:
...
async def validate_in_pipeline(
    ...
    try:
        ...
        logger.error("Pipeline validation failed: %s", e, exc_info=True)
