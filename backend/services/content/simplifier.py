            ...
            with torch.inference_mode():
                try:
                    ...
                    if (
                        ...
                        logger.warning("Sampling failed, falling back to greedy: %s", e)
                        fallback_config = {
                            "max_new_tokens": adaptive_max_tokens,
                            "use_cache": True,
                            "pad_token_id": self._tokenizer.eos_token_id,
...
class TextSimplifier:
    ...
    COMPLEXITY_THRESHOLDS: ClassVar[dict] = {
        "elementary": (0.0, 0.4),
        "middle": (0.4, 0.7),
        "secondary": (0.7, 1.0),
    }
    ...
    SUBJECT_TERMINOLOGY: ClassVar[dict] = {
        "Mathematics": [
            "equation",
            "variable",
            "coefficient",
    ...
    def __init__(
        ...
        if self.enable_refinement:
            ...
            logger.info("TextSimplifier initialized with %s", type(self.client).__name__)
    ...
    async def simplify_text(
        ...
        logger.info("Simplifying text for grade %s, subject %s", effective_grade, subject)
    ...
    async def _simplify_single_pass(
        ...
        try:
            ...
            logger.warning("LLM generation failed: %s, using rule-based fallback", e)
    ...
    async def _simplify_with_refinement(
        ...
        try:
            ...
            logger.warning("Initial generation failed: %s, using rule-based", e)
