...
def _get_translation_executor() -> ThreadPoolExecutor:
    """Get translation executor from unified ThreadPoolManager (lazy init)."""
...
def get_indictrans2_model():
    """Get or create singleton IndicTrans2 adapter using centralized singleton."""
    # Lock-free benign-race singleton.
    global _indictrans2_adapter
    if _indictrans2_adapter is not None:
    ...
    try:
        ...
        logger.warning("Could not initialize IndicTrans2 adapter: %s", e)
        _indictrans2_adapter = None
    return _indictrans2_adapter
...
class TranslationEngine:
    ...
    def translate(
        ...
        if use_cache:
            ...
            if cached is not None:
                logger.debug("Translation cache hit for %s", target_language)
                return cast("TranslatedText", cached)

        logger.info("Translating text to %s for subject %s", target_language, subject)
        ...
        if should_refine and REFINEMENT_AVAILABLE:
            ...
            try:
                ...
                logger.warning("Refinement failed, using single pass: %s", e)
                result = self._translate_single_pass(
                    text, target_language, subject, source_language
                )
    ...
    def _translate_single_pass(
        ...
        if self.model_client:
            try:
                ...
                logger.warning("Model inference failed, using fallback: %s", e)
    ...
    async def _translate_with_refinement(
        ...
        if self.model_client:
            try:
                ...
                logger.warning("Initial translation failed: %s", e)
        ...
        try:
            ...
            logger.error("Refinement pipeline failed: %s, returning initial output", e)
    ...
    def validate_script_rendering(self, text: str, language: str) -> bool:
        ...
        unicode_start, unicode_end = cast("tuple[int, int]", lang_info["unicode_range"])
    ...
    def _mark_technical_terms(
        ...
        logger.debug("Marked %s technical terms for preservation", len(term_map))
