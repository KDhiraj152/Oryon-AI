...
class AudioProcessor:
    ...
    def convert_format(
        ...
        try:
            ...
            logger.error("Audio format conversion failed: %s", e)
            return audio_content
    ...
    def optimize_for_speech(self, audio_content: bytes) -> bytes:
        ...
        try:
            ...
            logger.error("Speech optimization failed: %s", e)
            return audio_content
    ...
    def _trim_silence(self, audio: "AudioSegment") -> "AudioSegment":
        ...
        try:
            ...
            logger.warning("Silence trimming failed: %s", e)
            return audio
    ...
    def get_audio_info(self, audio_content: bytes) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("Could not extract audio info: %s", e)
    ...
    def validate_audio_quality(
        ...
        if AudioSegment:
            try:
                ...
                logger.debug("Could not analyze audio volume: %s", e)
...
class AudioCache:
    ...
    def __init__(self, cache_dir: str | None = None):
        self.cache_dir = Path(
            cache_dir if cache_dir is not None else os.getenv("AUDIO_CACHE_DIR", "data/cache/audio")
        )
    ...
    def _cleanup_old_files(self):
        ...
        for file_path in files_to_remove:
            try:
                file_path.unlink()
                logger.debug("Removed old cached audio: %s", file_path.name)
            except OSError as e:
                logger.warning("Failed to remove cached file: %s", e)
