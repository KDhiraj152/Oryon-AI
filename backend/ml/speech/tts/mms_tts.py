...
try:
    ...
    logger.info("MMS-TTS not available: %s", e)
...
class MMSTTSService:
    ...
    def _get_device(self) -> str:
        ...
        if DEVICE_ROUTER_AVAILABLE:
            try:
                ...
                logger.debug("Device router failed, using fallback: %s", e)
    ...
    def load_model(self, language: str = "en"):
        ...
        try:
            ...
            logger.debug("Memory coordinator not available for MMS-TTS: %s", e)

        logger.info("Loading MMS-TTS: %s on %s...", model_id, device)
        ...
        if coordinator:
            try:
                coordinator.register_model(
                    f"mms_tts_{lang}",
                    self.models[lang],
                    unload_fn=lambda lang_code=lang: self._unload_language(lang_code),  # type: ignore[misc]
                )
            except (RuntimeError, ValueError, OSError) as e:
        ...
        logger.info("MMS-TTS loaded for %s on %s", lang, device)
    ...
    def _unload_language(self, lang: str) -> None:
        ...
        try:
            ...
            pass
        logger.info("MMS-TTS unloaded language: %s", lang)
    ...
    def _normalize_language(self, language: str) -> str:
        ...
        if lang not in self.LANGUAGE_MAP:
            logger.warning("Language %s not supported, using English", lang)
            lang = "en"

        return lang

    @staticmethod
    def _encode_wav(waveform) -> bytes:
        """Encode a float32 numpy waveform to WAV bytes (CPU-bound)."""
        import io
        ...
        buf = io.BytesIO()
        sf.write(buf, waveform, samplerate=16000, format="WAV")
        return buf.getvalue()

    def _infer_waveform(self, text: str, lang: str):
        """Run GPU inference, return raw float32 numpy waveform."""
        import torch
        ...
        with torch.inference_mode():
            ...
            return output.waveform[0].cpu().float().numpy()
    ...
    def _synthesize_uncached(self, text: str, lang: str) -> bytes:
        ...
        return self._encode_wav(waveform)
    ...
    def synthesize(
        ...
        if use_cache:
            ...
            if cached is not None:
                logger.debug("TTS cache hit for %s", lang)
                return cached
    ...
    def warmup(self, languages: list[str] | None = None) -> None:
        ...
        logger.info("Warming up TTS for languages: %s", languages)
        for lang_code in languages:
            ...
            try:
                ...
                logger.info("TTS warmup complete for %s", lang)
            except (RuntimeError, ValueError, OSError) as e:
                logger.warning("TTS warmup failed for %s: %s", lang, e)
    ...
    def unload(self) -> None:
        ...
        logger.info("Unloading MMS-TTS models for %s languages...", len(self.models))
...
# Lock-free singleton with memory coordinator integration
_tts_service_instance: MMSTTSService | None = None

def get_mms_tts_service() -> MMSTTSService:
    """Get or create MMS-TTS service singleton (with memory coordination)."""
