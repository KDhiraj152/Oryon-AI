...
class UnifiedInferenceEngine:
    ...
    DEFAULT_SYSTEM_PROMPT = """You are Oryon, an multilingual AI assistant.
...
For code: Use proper code blocks with language tags"""
        ...
        with self._lock:
            ...
            if routing.backend == ComputeBackend.MLX:
                ...
                while len(self._mlx_engines) >= self._max_loaded_models:
                    ...
                    logger.info("[UnifiedEngine] Evicting model '%s' to load '%s'", oldest, model_key)
                ...
                self._mlx_load_order.append(model_key)
                logger.info("[UnifiedEngine] Loaded model '%s' (pool: %s)", model_key, list(self._mlx_engines.keys()))
    ...
    async def generate(
        ...
        try:
            ...
            logger.error("[UnifiedEngine] Generation error: %s", e)
            raise
    ...
    async def _generate_pytorch(
        ...
        is_preformatted = "<|im_start|>" in prompt or "<|system|>" in prompt or "<|user|>" in prompt
        ...
        # FIX: Use dedicated GPU executor (2 workers) instead of default
        # unbounded executor to prevent thread explosion under load.
        from . import get_gpu_executor

        return await loop.run_in_executor(get_gpu_executor(), _generate)
