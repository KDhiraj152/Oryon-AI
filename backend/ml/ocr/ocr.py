...
class TesseractOCR:
    ...
    def ocr_with_confidence(
        ...
        try:
            ...
            logger.error("Tesseract OCR with confidence failed: %s", e)
            return "", 0.0
...
class GOTOCR2:
    ...
    def __init__(self, model_id: str | None = None, device: str | None = None):
        ...
        logger.info("GOT-OCR2 initialized: %s on %s", self.model_id, self.device)
    ...
    def _detect_best_device(self) -> str:
        ...
        if HARDWARE_OPT_AVAILABLE:
            try:
                ...
                logger.debug("Hardware optimizer failed, using fallback: %s", e)

        # Check CUDA (NVIDIA GPU)
        if torch.cuda.is_available():
            gpu_mem = torch.cuda.get_device_properties(0).total_memory / (1024**3)
            logger.info("CUDA available with %.1fGB VRAM", gpu_mem)
            return "cuda"
        ...
        if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            ...
            try:
                ...
                logger.info("MPS (Metal) available on %s - using GPU acceleration", chip)
                return "mps"
            except (RuntimeError, OSError) as e:
                logger.warning("MPS available but not functional: %s", e)
    ...
    def _load_model(self):
        ...
        logger.info("Loading GOT-OCR2 model: %s on %s", self.model_id, self.device)
        ...
        try:
            ...
            logger.debug("Memory coordinator not available for GOT-OCR2: %s", e)
        ...
        try:
            ...
            if coordinator:
                try:
                    ...
                    logger.debug("Memory coordinator registration failed: %s", e)

            # MPS shader warmup
            if self.device == "mps":
                self._warmup_mps()

            logger.info("GOT-OCR2 model loaded successfully on %s", self.device)

        except (ImportError, RuntimeError, OSError, ValueError) as e:
            logger.error("Failed to load GOT-OCR2: %s", e)
    ...
    def _warmup_mps(self):
        ...
        try:
            ...
            logger.warning("MPS warmup failed (non-critical): %s", e)
    ...
    def ocr_image(
        ...
        try:
            ...
            logger.error("GOT-OCR2 inference failed: %s", e)
...
class OCRService:
    ...
    SUPPORTED_LANGUAGES: ClassVar[list] = [
        "Hindi",
        "Tamil",
        "Telugu",
        "Bengali",
    ...
    def __init__(
        ...
        if self._use_tesseract:
            logger.info("OCRService using Tesseract (languages: %s)", self.languages)
        else:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if self._got_available:
                    ...
                    logger.warning("GPU has %.1fGB VRAM, GOT-OCR2 needs 6GB+", gpu_mem)
                return self._got_available
            ...
            logger.warning("Could not check GPU availability: %s", e)
            self._got_available = False
    ...
    def extract_text(
        ...
        try:
            ...
            logger.warning("GOT-OCR2 failed, falling back to Tesseract: %s", e)
            return self._extract_with_tesseract(file_path, file_ext)
    ...
    def _extract_with_tesseract(
        ...
        logger.info("Using Tesseract for: %s", file_path)
    ...
    def _process_pdf_with_tesseract(self, pdf_path: str) -> ExtractionResult:
        ...
        try:
            ...
            logger.error("Tesseract PDF processing failed: %s", e)
            raise
    ...
    def validate_extraction(self, result: ExtractionResult) -> bool:
        ...
        if result.confidence < 0.60:
            logger.warning("Low OCR confidence: %.2f", result.confidence)
            return False

        return True

# Lock-free singletons with memory coordinator integration
