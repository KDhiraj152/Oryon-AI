    ...
    async def _rerank_collaboration(
        ...
        for i in range(num_candidates):
            try:
                ...
                logger.warning("[Rerank] Candidate %s generation failed: %s", i + 1, e)
        ...
        if reranker and len(candidates) > 1:
            try:
                ...
                logger.warning("[Rerank] Reranking failed: %s, using first candidate", e)
                best_score = 0.7
        else:
...
Generate a high-quality output for the above task."""
            ...
            logger.warning("[Collaborator] LLM generation failed: %s", e)
            return None
