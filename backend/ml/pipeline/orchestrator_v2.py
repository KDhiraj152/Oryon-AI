...
from backend.models import ProcessedContent
...
class ConcurrentPipelineOrchestrator:
    ...
    SUPPORTED_SUBJECTS: ClassVar[list] = [
        "Mathematics",
        "Science",
        "Social Studies",
        "English",
    ...
    STAGE_CONCURRENCY: ClassVar[dict] = {
    ...
    async def process_content(
        ...
        Raises:
            OryonException: If validation fails
        """
        # Validate inputs
        ...
        try:
            ...
            with self.perf_monitor.span("pipeline_execution"):
                ...
                logger.info("Pipeline completed successfully: %s", content_id)
            ...
            raise OryonException("Pipeline processing timeout", status_code=504)
    ...
    async def _execute_stage_with_backpressure(
        ...
        async with self.semaphores[stage]:
            try:
                ...
                logger.debug("Stage %s completed in %sms", stage.value, elapsed_ms)
                ...
                logger.error("Stage %s timeout", stage.value)
    ...
    def _validate_parameters(
        ...
        if not input_data:
            raise OryonException("Input data is empty", status_code=400)
