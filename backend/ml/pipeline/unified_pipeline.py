...
Format: SCORE: X/10 | FEEDBACK: [your feedback]"""
    ...
    def _get_translation_engine(self):
        ...
        if self._translation_engine is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load IndicTrans2: %s", e)
    ...
    def _get_validation_module(self):
        ...
        if self._validation_module is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load validation: %s", e)
    ...
    def _get_embedder(self):
        ...
        if self._embedder is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load embedder: %s", e)
    ...
    def _get_reranker(self):
        ...
        if self._reranker is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load reranker: %s", e)
    ...
    def _get_tts_service(self):
        ...
        if self._tts_service is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load TTS: %s", e)
    ...
    def _get_stt_service(self):
        ...
        if self._stt_service is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load STT: %s", e)
    ...
    def _get_ocr_service(self):
        ...
        if self._ocr_service is None:
            try:
                ...
                logger.warning("[UnifiedPipeline] Could not load OCR: %s", e)
    ...
    async def _run_parallel_tasks(
        ...
        for _i, (name, task_result) in enumerate(
            ...
            if isinstance(task_result, Exception):
                logger.warning("[Pipeline] %s task failed: %s", name, task_result)
                continue
            if name == "translate":
    ...
    async def process(
        ...
        try:
            ...
            if cached:
                logger.info("[Pipeline] Cache hit for %s", request.request_id)
            ...
            logger.error("[Pipeline] Error in %s: %s", request.request_id, e)
    ...
    async def _process_fast_path(
        ...
        try:
            ...
            logger.error("[Pipeline] Fast path error in %s: %s", request.request_id, e)
            result.success = False
            result.error = str(e)
    ...
    async def _stream_parallel_tasks(
        ...
        for key, res in zip(keys, results, strict=False):
            if isinstance(res, Exception):
                logger.warning("[Pipeline-Stream] %s failed: %s", key, res)
                continue
            output[key] = res
        return output
    ...
    async def _translate(
        ...
        if cached:
            logger.debug("[Pipeline] Translation cache hit for %s", target_language)
            return str(cached)
        ...
        if translation_engine and translation_engine.model_client:
            try:
                ...
                # FIX (serialization bottleneck): translation_engine.translate()
                # is synchronous (GPU-bound torch MPS inference). Offload to
                # executor so the event loop stays responsive and parallel
                # validation task can actually run concurrently.
                loop = asyncio.get_running_loop()
                trans_result = await loop.run_in_executor(
                    None,
                    lambda: translation_engine.translate(
                        text=text,
        ...
        if result is None:
            logger.info("[Pipeline] Using LLM for %s translation", target_language)
...
{target_language} translation:"""
    ...
    def _validate_with_gemma_sync(
        ...
        try:
            ...
            logger.warning("[Pipeline] Qwen3-8B validation failed, using LLM: %s", e)
            return None, ""
    ...
    async def _generate_audio(
        ...
        if cached:
            logger.debug("[Pipeline] TTS cache hit for %s", language)
        ...
        try:
            ...
            if tts_service:
                logger.info("[Pipeline] Using MMS-TTS for %s audio", language)
            ...
            logger.warning("[Pipeline] Audio generation failed: %s", e)
            return None
    ...
    async def ocr(
        ...
        try:
            ...
            logger.info("[Pipeline] OCR processing %s: %s", file_ext, file_path)
            ...
            logger.warning("[Pipeline] OCR failed: %s", e)
        ...
        if ocr_service and file_ext in [".png", ".jpg", ".jpeg", ".bmp", ".webp"]:
            try:
                ...
                logger.warning("[Pipeline] Legacy OCR failed: %s", e)
    ...
    async def transcribe(
        ...
        if stt_service:
            try:
                logger.info("[Pipeline] Transcribing audio: %s", audio_path)
                ...
                logger.warning("[Pipeline] Whisper transcription failed: %s", e)
