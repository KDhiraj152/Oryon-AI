...
class ContentPipelineOrchestrator:
        ...
        try:
            ...
            if output_format in ['audio', 'both']:
                try:
                    ...
                    # Speech generation is non-critical - log warning and continue
                    logger.warning(f"Speech generation skipped: {e}")
                    logger.info("Continuing pipeline without audio output")
            
            # Store processed content in database (before validation)
            ...
            add_breadcrumb(
                message="Starting curriculum validation",
                category="pipeline",
                level="info",
                data={"content_id": str(content_id), "subject": subject}
            )
            ...
            try:
                ...
                validation_passed, validation_summary = asyncio.run(validate_in_pipeline(
                    db=db,
                    content=content,
                    text=simplified_text  # Validate simplified content
                ))
                ...
                add_breadcrumb(
                    ...
                    data={
                        ...
                        "score": validation_summary.get("overall_score", 0.0)
                    }
                )
                
                logger.info(
                    f"Validation {'PASSED' if validation_passed else 'FAILED'}: "
                    f"score={validation_summary.get('overall_score', 0.0):.2%}"
                )
                ...
                capture_exception(
                    e,
                    context={"content_id": str(content_id), "stage": "validation"},
                    tags={"pipeline": "orchestrator", "stage": "validation"}
                )
    ...
    def _generate_speech(
        ...
        # Use the SpeechGenerator component
        ...
        return audio_file.file_path, audio_file.accuracy_score or 0.90
