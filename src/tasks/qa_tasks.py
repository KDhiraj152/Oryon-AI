    ...
    try:
        ...
        try:
            content = session.query(ProcessedContent).filter(
                ProcessedContent.id == UUID(content_id)
            ).first()
...
def answer_question_task(
    ...
    """
    Answer a question about a document using RAG + Flan-T5.
...
Answer the question based only on the context provided above. If the answer cannot be found in the context, say "I cannot answer based on the given context."

Answer:"""
        
        # Get Flan-T5 model and generate answer
        logger.info("Generating answer with Flan-T5...")
        simplifier = get_text_simplifier()
        
        # Use the model to generate answer
        # Note: TextSimplifier doesn't have a generate_text method, so we'll use simplify
        # which internally uses the T5 model
        answer = simplifier.simplify_text(
            text=prompt,
            grade_level=8,  # Use middle school level for general answers
            subject="General"
        ).text
        ...
        logger.info(f"Question answered successfully (confidence: {confidence_score:.2f})")
